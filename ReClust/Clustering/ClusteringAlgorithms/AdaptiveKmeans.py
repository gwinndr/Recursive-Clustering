...
# Adaptive Kmeans info class
class AdaptiveKmeansReclust_c:
    def __init__(self):
        self.mMaxClusterNum     = 20 # Tweaks for silhoutte method
        self.mAlgorithmName     = "Adaptive Kmeans"
        self.mSaveLocation      = "./Outputs/AdaptiveKmeans_Sil_" + str(self.mMaxClusterNum)
        ...
        self.mMaxClusterRange       = self.mMaxClusterNum + 1
    ...
    # Adaptive Kmeans parameters here for Reclust (REQUIRED)
    def ReclusterFunction(self, cluster):
        ...
        if(self.mMaxClusterRange > num_points):
            actual_max_clusters = num_points
        else:
            actual_max_clusters = self.mMaxClusterRange

        cluster_solution_nums = range(2, actual_max_clusters)
        for n_clusters in cluster_solution_nums:
            kmeans = KMeans(n_clusters=n_clusters, precompute_distances=True).fit(np_coords)
